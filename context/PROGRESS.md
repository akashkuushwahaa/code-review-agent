...
| 3 | RAG context | In Progress | 2026-07-22 | | Step A (full-file context) done: F1 0.824 → 0.938 A/B on same 18 cases; Step B (Chroma) awaiting review |
...
**Active phase:** Phase 3 — RAG context. Step A (full-file context) is done and
measured (F1 0.824 → 0.938, A/B on the same 18 cases). Step B (Chroma
retrieval) is deliberately held until Step A is reviewed, per this phase's
scope. Phase 1 still needs its live-trigger verification (repo secret + a real
PR run).
